...
void lerBancoDeDados(int id);
void leitorSaindo(void);
void escritorPedindoAcesso(int id);
void escreverBancoDeDados(int id);
...
int main()
{
    ...
    {
        pthread_create(&(threadsdosescritores[i]),NULL, escritor ,&(i));
    }
    ...
    for(i=0 ; i<LEITOR ; i++)
    {
        int calma;
        calma = rand() % 2;
        pthread_create(&(threadsdosleitores[i]),NULL, leitor ,&(i));
        sleep(calma);
    }
...
void lerBancoDeDados(int id)
{
    ...
    printf("O leitor %d Lendo o Banco de dados. Um total de %d.\n\n Lendo %d\n\n",id,leitor_lendo,num);
...
void escreverBancoDeDados(int id)
{
    ...
    num=num+1;
  printf("O escritor %d esta escreverndo. \n", id);
  printf(" \n Escrevendo %d\n\n",num);
